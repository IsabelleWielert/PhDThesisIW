function msd(tr, data_all)
     ...
     for k=1:tr_max
        ...
        [param] = fit(1/10*(3:60)', msd(3:60)', my_msd, opts);
        figure(1000)
        ...
        hold on
        if param.alpha > 0.9
            alpha(num_alpha,:) = [k,param.alpha];
            num_alpha = num_alpha + 1
        end
     end
     
     path = cd;
     text(10,10, ['Scaling Exponent:\newline', num2str(mean(alpha(:,2)), '%1.3f')],'FontSize',12),
     ...
     path_alpha = [path,'\Scaling_Exponents_for_selected_tracks.mat'];
     save(path_alpha, 'alpha', '-mat');
